...
float3 calculate_secondary( float2 uv, float3 vColor, float2 vPos )
{
	...
	return (vColor * ( 1.0f - vSecondary.a ) + ( vSecondColor / nDivisor ) * vSecondary.a) * 1.75f;
}
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 vOut = CalculateLighting( sample.rgb*1.75f, normal );

	vOut = ApplyDistanceFog( vOut, Input.prepos, FoWTexture, FoWDiffuse );

	return float4( ComposeSpecular( vOut, 0.0f ), 2.0f );
}
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	vOut = CalculateLighting( vOut*1.75f, normal );
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 vOut = CalculateLighting( sample, normal * vFog * 0.18f );
	return float4( vOut*2.2f, 1.0f );
}
